...
//	var textToSpeechURL = "http://api.voicerss.org/?key=[YOUR_KEY]&hl=en-gb&c=ogg&src="; // the text to speech converter
	...
	var advance = -100; // advance to next slide after given time in milliseconds after audio has played, use negative value to not advance
	var autoplay = true; // automatically start slideshow
	var playerOpacity = .00; // opacity when the mouse is far from to the audioplayer
	var startAtFragment = false; // when moving to a slide, start at the current fragment or at the start of the slide
	var playerStyle = "display:none"; // style used for container of audio controls
	// ------------------
	...
	Reveal.addEventListener( 'fragmentshown', function( event ) {
		if ( timer ) { clearTimeout( timer ); timer = null; }
            console.debug( "fragmentshown-audio ");
		selectAudio();
	} );
